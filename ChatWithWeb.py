...
def get_vector_store_from_urls(urls):
    all_documents = []
    for url in urls:
    ...
    vector_store = FAISS.from_documents(all_documents, HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2"))
    return vector_store
...
with st.sidebar:
    urls_input = st.text_area("Add Webpage URLs (one per line)", value="\n".join(st.session_state.urls))
    ...
    if process_button:
        ...
        if urls != st.session_state.urls:
            st.session_state.urls = urls

            with st.spinner("Processing..."):
                try:
                    st.session_state.vector_store = get_vector_store_from_urls(urls)
...
if st.session_state.file_processed:
    ...
    st.info("Please provide webpage URLs to start chatting.")
